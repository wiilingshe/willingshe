var addObj = {},//addComponent
		...
		tplCount = 0,
		sceneId = window.location.pathname;
var vxuebao = function(){
	var  reset = function(a) {//按照z-index值重新排序
		...
		makeLi = function(a, b) {//拼接每一个素材li
			...
			if (c) {
				...
				/*if (*/
					...
					})/*, "edit" == data.mode || "x" != ("" + a.type).charAt(0)) {*/
						var innerCon = $('<div class="element-box-contents"></div>');
						var outerCon = $('<div class="element-box"></div>');
						var f = outerCon.append(innerCon.append(c));
				...
				//判断移动端&&修改大小
					if (a.css) {
						 var g = PCcheck() ? (320*(checkHeight()) - parseInt(a.css.left)) : (320 - parseInt(a.css.left));
						/*if (*/
						e.css({
							width: g
						}), e.css({
						...
						}), /*(0 === a.css.boxShadowSize || "0" == a.css.boxShadowSize) && (a.css.boxShadow = "rgba(0,0,0,0) 0 0 0"), "edit" != data.mode && "x" == ("" + a.type).charAt(0)) {
							return e.append(c)
						}, */
		...
		turnTo = function(elem, da) {//跳转
			"view" == data.mode && da.properties.url && $(elem).click(function (a) {
				var c = da.properties.url;
		...
		getEffect = function(list,type){
			...
			switch (type) {
				case "finger":
					...
					return $.getScript(CLIENT_CDN + "view/js/lock_effect.js", function (b, c, d) {
								test(audioObj, pages, pageInfo, $(".m-img").width(), $(".m-img").height(),a)
							});
				case "scratch":
					//alert(222);
					return $.getScript(CLIENT_CDN + "view/js/scratch_effect.js", function (b, c, d) {
								addScratchEffect(audioObj, pages, a, a)
							});
				case "fallingObject":
					//alert(333);
					return $.getScript(CLIENT_CDN + "view/js/falling_object.js", function (b, c, d) {
				...
				case "effect":
					...
					return resources.load(window.eqx[list.list[0].properties.effect.name].config.resources), resources.onReady(function () {
								window[list.list[0].properties.effect.name].doEffect(audioObj, a, list.list,renderPage, a)
							});
			}
		...
		playPageMusic = function(pageMusics){//播放当页音乐
			...
			var nowPage = $("#page1");
			var pageNum = 1;
			var src = pageMusics[0] && (pageMusics[0].url.substr(0,5) == "index" ? pageMusics[0].url.split('index.php')[1] :("Uploads/" + pageMusics[0].url));
	...
	vxuebao.addInterceptor(function(a, b, c) {
		...
		b.id = a.id, b.setAttribute("class", "element comp_title"), a.content && (b.textContent = a.content);
		if (a.css) {
			var c, d = a.css;
			for (c in d) {
				b.style[c] = d[c]
		...
		if(PCcheck()){//判断移动端&&修改大小
			if(a.content.indexOf("font-size") >= 0){
				...
				while((arr = re.exec(content)) !=null){ //exec使arr返回匹配的第一个，while循环一次将使re在g作用寻找下一个匹配。 
					...
					var fontSize = parseInt(Number(a.content.slice(position+11,position+13))*(checkHeight()));
		...
		if(a.properties.imgSrc){
			...
			if(/^http.*!/.test(e)){
				f.src = e, d.style.backgroundImage = "url(" + e + ")";
			}else{
				f.src = window.PREFIX_FILE_HOST+ e, d.style.backgroundImage = "url(" + window.PREFIX_FILE_HOST + e + ")";
			}
		...
		if(/^http.*/.test(a.properties.src)){
			b.setAttribute("videourl", a.properties.src);
		}else{
			b.setAttribute("videourl", PREFIX_FILE_HOST + a.properties.src);
		}
		...
		if (a.properties && a.properties.children) {//判断移动端&&修改大小
			a.css.width = !PCcheck() ? a.css.width : Math.round(parseInt(a.css.width)*(checkHeight()));
			a.css.height = !PCcheck() ? a.css.height : Math.round(parseInt(a.css.height)*(checkHeight()));
			var parentWidth = a.css.width,
				parentHeight = a.css.height,
				e = $('<div id="' + a.id + '" class="slider element" ctype="' + a.type + '"></div>');
			a.properties.bgColor && e.css("backgroundColor", a.properties.bgColor);

			$.each(a.properties.children, function (index, item) {
				var g = vxuebao.getPictureList(item.width, item.height, parentWidth, parentHeight),
					h = $('<img src="' + window.PREFIX_FILE_HOST + item.src + '">');
				h.css({//判断移动端&&修改大小
					margin: (parentHeight - g.height) / 2 + "px " + (parentWidth - g.width) / 2 + "px",
		...
		d.append(e);
		if("view" == data.mode){
			counterValues && counterValues.then(function (c) {
				var d = c.map[b.id] || 0;
					...
					if ($("textarea", d).each(function () {
							if (e) {
								if ("required" == $(this).attr("required") && "" == $(this).val().trim()) {
									layer.alert($(this).attr("placeholder") + c.REQUIRED);
									return void(e = !1);
								}
	...
	function getData() {//接收后台数据
		var isNewPreviewLocation = /[http|https]:\/\/.*\/m\/scene\/preview\//.test(window.location.href);
		var url = /[http|https]:\/\/.*\/v-/.test(window.location.href) ? window.location.href.split("/v-")[1] : isNewPreviewLocation ? window.location.href.split("/m/scene/preview/")[1] : window.location.href.split("id=")[1];
		//console.log(scene.id);
		var sceneId = url.split("#")[0].split("&")[0].split("?")[0];
		var a = PREFIX_URL + "index.php?c=scene&a=view&diyname="+scene.diyname+"&id="+scene.id+"&time="+(new Date).getTime();
		$.ajax({
			type: "GET",
			url: a,
			xhrFields: {
			...
			var obj = {
				bgAudio: eval("("+a.obj.bgAudio+")")
			},effectType;
			getCounterValues(a),getShowCount(a.obj.id);
			data.def = a.list[0];
			if(!isEmpty(a.list[0].properties)){
				if(!isEmpty(a.list[0].properties.finger)){
					effectType = "finger";
				}else if(!isEmpty(a.list[0].properties.scratch)){
					effectType = "scratch";
				}else if(!isEmpty(a.list[0].properties.fallingObject)){
					playVideo(obj);
					effectType = "fallingObject";
				}else if(!isEmpty(a.list[0].properties.effect)){
					playVideo(obj);
					effectType = "effect";
				}else{
					playVideo(obj);
				}
				vxuebao.getEffect(a,effectType);
			}else{
				playVideo(obj);
			}; 
			vxuebao.playPageMusic(a.obj.music_url);
			vxuebao.parse(data);
			mobilecheck() && ($("#code").remove(),$(".viewer").css({
				width: "100%",
				height: "100%",
				margin: 0
			}),$(".edit_wrapper").css("width","100%"),$("#container").css("width","100%"),renderPage(vxuebao,1,data));
			(parseInt(JSON.parse(a.obj.property).eqAdType) == 1) && takeAd();
		}).then(function(){
			window.setTimeout(
				function() {
