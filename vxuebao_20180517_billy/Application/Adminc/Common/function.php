...
function getTpyeNameToId($mode){
	$array=array('tpType'=>1,'bgType'=>0,'musType'=>33  ,'scene_type'=>2,'mytpl_type'=>1);
	return $array[$mode];
}
...
function getSceneTag($biztypeId){
	...
	$title=M('dtag')->where("value=".$biztypeId)->getField('name');
	return $title;
}
...
// 缩略图
function thumb_img($data,$datapath,$dataname,$width,$height,$type){
	$img = new \Think\Image();
	
	$big = $upload->rootPath.$data;
	return $upload->rootPath;
	
	/*$img->open($upload->rootPath.$datapath.$dataname);
	$img->thumb($width,$height,$type);
	$small_photo = $upload->rootPath.$datapath.'small'.$dataname;
