...
$this->assign('ui',$ui);
	...
	/*	$where['fileid_bigint']=I('get.id');
		...
		if(IS_POST){
			...
			$this->success ( '操作成功', '/adminc.php?s=/file/index/filetype'.'/'.I("post.fileType").'/'.'p'.'/'.I("get.p"));
			...
			$biztypeId=$userinfo['biztype_int'];
	 ...
	 public function del(){
		...
		if (I('get.flag') == 'useranli') {
			...
			$this->success('操作成功','/adminc.php?s=/file/index/filetype'.'/'.I("get.filetype").'/'.'p'.'/'.I("get.p"));
	...
	public function add(){
		...
		if(IS_POST){
			...
			if($type<2){
				...
				$data[$i]['filethumbsrc_varchar']=str_replace('Uploads/','',$photo[$i]);
				$data[$i]['filename_varchar']=str_replace('Uploads/','',$filename[$i]);
				$data[$i]['ext_varchar']=substr(strrchr($photo[$i], '.'), 1);; 
				$data[$i]['filetype_int']=I('get.filetype');
				$data[$i]['biztype_int']=I('POST.dtagid');
				$data[$i]['tagid_int']=I('POST.tagid');
				$data[$i]['filesrc_varchar']=str_replace('Uploads/','',$photo[$i]);
