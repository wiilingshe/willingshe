...
<div class="container-fluid">
    ...
    <div class="tabbable tabbable-custom tabbable-full-width">
        ...
        <form class="login-form" action="<?php echo U('user/index');?>" method="post" id="form">
            根据用户名搜索(支持模糊查询)<select name="type">
              <option value="0">所属用户组</option>
			  <?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"  <?php echo ($v['id']==$type?'selected="selected"':''); ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>&nbsp&nbsp&nbsp&nbsp <input type="text" name="username" id="id"  value="<?php echo ($username); ?>" />
            ...
            <div class="portlet-body">
                ...
                <div class="portlet-body form ">
                    <?php if(is_array($select)): foreach($select as $key=>$user): ?><div class="media-body">
                            <!-- Nested media object -->
                            <div class="media">
                                <a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>"class="pull-left">
                                    <img src="<?php if(substr($user[headimg],0,4) == "http" ): echo ($user["headimg"]); else: ?>/Uploads/<?php echo ((isset($user["headimg"]) && ($user["headimg"] !== ""))?($user["headimg"]):"defaultuser.jpg"); endif; ?>" width="60" height="60" class="media-object" style=" width:60px; height:60px"/> </a>
                                <div class="media-body">
                                    <h4 class="media-heading">
                                        <strong class="btn purple-stripe">登陆名：<?php echo ($user["email_varchar"]); ?></strong>   
                                        <strong class="btn purple-stripe">用户组：<?php echo ($user_group[$user['type']]); ?></strong>
                                        <strong class="btn purple-stripe">微币：<?php echo ($user["xd"]); ?></strong>
                                        <a class="btn purple-stripe" href="/adminc.php?c=Scene&a=index&user_id=<?php echo ($user["userid_int"]); ?>">已创建微课</a>
                                        <strong class="btn purple-stripe">剩余导出次数：<?php echo ($user["export_num"]); ?></strong>
