...
<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li><a href="#">开放接口管理</a><i class="icon-angle-right"></i></li>
						</ul>
					</div>
				</div>
                ...
                <div class="tabbable tabbable-custom tabbable-full-width" style="margin-bottom:0px; padding-bottom:0px">
					...
					<li class="active"> <a href="<?php echo U('Open/index');?>"> 开放接口管理</a> </li>
					</ul>
					<hr>
                </div>
				<!-- BEGIN PAGE CONTENT-->          
				<div class="row-fluid">
					<div class="span12 ">
						...
						<div class="cf">
						<a id="export" class="btn green" href="<?php echo U('Open/edit');?>" autocomplete="off"> <i class="icon-plus"></i> 添加接入</a>
						</div>
...
<table class="table table-striped table-bordered table-hover dataTable" >
	...
	<tbody><?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			<td><?php echo ($v["id"]); ?></td>
			<td><?php echo ($v["app_name"]); ?></td>
			<td><?php echo ($v["app_key"]); ?></td>
			<td><?php echo ($v["app_secret"]); ?></td>
			<td><?php echo (date("Y-m-d H:i:s",$v["creatime"])); ?></td>
			<td><?php if(($v["ok"]) == "1"): ?>启用<?php else: ?>禁用<?php endif; ?></td>
			<td class="action"><a class="ajax-get no-refresh" href="<?php echo U('Open/edit?id='.$v['id']);?>">修改</a>&nbsp;<a class="ajax-get no-refresh" href="<?php echo U('Open/del?id='.$v['id']);?>">删除</a>
			</td>
