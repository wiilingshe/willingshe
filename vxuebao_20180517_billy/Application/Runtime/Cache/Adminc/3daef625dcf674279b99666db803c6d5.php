...
<div class="row-fluid"
	...
	<div class="portlet-body" style=" margin-top:15px;">
            ...
            <?php if(is_array($select)): foreach($select as $key=>$item): ?><tr>
                ...
                <td align="center" ><a  href="?c=article&a=e&id=<?php echo ($item["id"]); ?>" ><i class="btn black"><i class="icon-edit icon-white"></i></i></a> <?php if(!in_array(($item["id"]), explode(',',"79"))): ?><a href="javascript:;" onClick="javascript:if(confirm('你确信要删除[<?php echo ($item["title"]); ?>]吗？')) window.location='?c=article&a=del&id=<?php echo ($item["id"]); ?>&type=<?php echo ($type); ?>'"><i class="btn black"><i class="icon-remove icon-white"></i></i></a><?php endif; ?>
                ...
                <td> <?php switch($item["type"]): case "1": ?>关于我们<?php break;?>  
                                    ...
                                    <?php case "8": ?>帮助中心<?php break;?>    
									<?php case "9": ?>留言<?php break;?>
									<?php case "10": ?>用户协议<?php break;?>    
