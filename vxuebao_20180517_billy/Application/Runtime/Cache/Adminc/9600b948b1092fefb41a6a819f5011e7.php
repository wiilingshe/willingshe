    ...
    <div class="row-fluid">
        <div class="tabbable tabbable-custom tabbable-full-width">
            <ul class="nav nav-tabs">
				...
				<li><a href="<?php echo U('Scene/syspageimport');?>">导入单页模板</a></li>
        ...
        <div class="tab-content">
            <div class="row-fluid search-forms search-default" style="visibility: hidden;">
                <form class="form-search" action="<?php echo U('Scene/pagesys');?>" method="post" id="form">
                    <div class="chat-form">
                        <div class="input-cont">                               
                            ...
                            <span>
                                ...
                                <select style='width:120px'  class="form-control" name="order">
                                    <option value="pageid_bigint" <?php echo ($select_str_order[pageid_bigint]); ?>>ID编号</option>
                                    <option value="listorder" <?php echo ($select_str_order[listorder]); ?>>自定义排序</option>
                                </select>
                                <label for="">
                                    排序规则 
                                </label>
                                <select style='width:120px'  class="form-control" name="sort">
                                    <option value="desc" <?php echo ($select_str_sort[desc]); ?>>倒序</option>
                                    <option value="asc" <?php echo ($select_str_sort[asc]); ?>>正序</option>
                                </select>
            ...
            <div class="row-fluid portfolio-block">         
                ...
                <form name="myform" action="" method="post" target="_blank">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><tr>
							<td><input name="id[]" type="checkbox" value="<?php echo ($r["pageid_bigint"]); ?>" /></td>
                            <td><input type="text" value="<?php echo ($r["listorder"]); ?>" name="listorder[<?php echo ($r["pageid_bigint"]); ?>]" style="width:50px;" /></td>
                            <td><?php echo ($r["pageid_bigint"]); ?></td>
                            <td><img src="/Uploads/<?php echo ($r["thumbsrc_varchar"]); ?>" width="100" /></td>
                            <td><?php echo ($r["pagename_varchar"]); ?></td>
                            <td><?php echo ($r["tagstr"]); ?></td>
                            <td><a href="?c=Scene&a=editpagesys&pageid=<?php echo ($r["pageid_bigint"]); ?>&p=<?php echo $_GET['p']?>" >编辑</a> <a onclick="return confirm('是否删除') ? true : false;" href="?c=Scene&a=delpagesys&pageid=<?php echo ($r["pageid_bigint"]); ?>" >删除</a></td>
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="6"><input type="button" value="排序" onclick="document.all.myform.action='<?php echo U('Scene/listorder');?>';myform.submit();" class="btn green"/>    <input type="button" value="导出所选单页模板" onclick="document.all.myform.action='<?php echo U('Scene/syspageexport');?>';document.all.myform.target='_blank';myform.submit();" class="btn green"/></td>
                        </tr>
                    </tfoot>
                </table>         
            </div>
