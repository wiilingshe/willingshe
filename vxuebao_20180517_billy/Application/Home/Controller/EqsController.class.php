		...
		public function scene_template_prices(){
			...
			public function banners(){
				if(I('get.pageCode')=='ad_001'){
					/*echo '{"success":true,"code":200,"msg":"操作成功","obj":null,"map":null,"list":[{"id":602,"path":"/Uploads/ad/2015-08-11/gg1.jpg","title":"免费升级企业账号，尊享更多专属权限","url":"http://www.hdb.com/party/1egc-comm.html","target":"_blank","pageCode":"ad_001","sort":"4","status":1},{"id":1202,"path":"/Uploads/ad/2015-08-11/gg2.png","title":"用秀点去易企秀尾页，点此购买","url":"http://www.learnnow.net.cn/syb.html","target":"_blank","pageCode":"ad_001","sort":"3","status":1}]}';*/
			
					$banners=M('ad');
         			$where['type']=6;
          			$links=$banners->where($where)->select();
          			$jsonstr ='{"success":true,"code":200,"msg":"操作成功","obj":null,"list":[';
		            foreach ($links as $var)
		            {
		              $jsonstrtemp = $jsonstrtemp.'{"id":"'.$var['id'].'","title":"'.$var['descript'].'","url":"'.$var['link'].'","path":"/Uploads'.$var['url'].'","target":"_blank","pageCode":"ad_001","sort":"'.$var['sort'].'","status":1},';  
		            }
		              $jsonstr = $jsonstr . rtrim($jsonstrtemp, ',') . ']}';
			...
			echo '{"success":true,"code":200,"msg":"操作成功","obj":null,"map":null,"list":[{"id":3501,"path":"/assets/images/show/1.jpg","title":"","url":"/","target":"_blank","pageCode":"index","sort":"23","status":1}]}';
    //}	
...
if(I('get.all')=='1'){
	...
	public function promotion(){
		...
		$_scene_list=$_scene->where($where)->order($order)->page(I('get.pageNo',1),$pageshowsize)->select();
		//print_r($_scene->getlastsql());die; 
		$jsonstrtemp = '';
		foreach($_scene_list as $vo){
			...
			"us_name":'.json_encode($vo['uname']).',
			"desc_varchar":'.json_encode($vo['desc_varchar']).',
            "image": {
                "bgAudio": {
	...
	public function promotionpc(){
		...
		$_scene_list=$_scene->where($where)->join('left join cj_users on cj_scene.userid_int=cj_users.userid_int')->order('rank desc,sceneid_bigint desc')->page(I('get.pageNo',1),$pageshowsize)->select();
