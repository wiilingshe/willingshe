...
class AdController extends Controller {
     ...
     public function product(){
        $friend=M('ad');
          ...
          $jsonstr ='{"success":true,"code":200,"msg":"操作成功","list":[';
            foreach ($links as $var)
            {
              $jsonstrtemp = $jsonstrtemp.'{"descript":"'.$var['descript'].'","link":"'.$var['link'].'","url":"/Uploads'.$var['url'].'"},';  
            }
              $jsonstr = $jsonstr . rtrim($jsonstrtemp, ',') . ']}';
              echo $jsonstr;
    }       
    ...
    public function getBanner70(){
		...
		$friend=M('ad');
		$where['type']=2;
		$links=$friend->where($where)->order("sort desc,id desc")->select();
      ...
      if($var['link'] != ''){
			  $jsonstr.='<li  style="display:  '.$display.'; background: url(/Uploads'.$var['url'].') 50% 0 no-repeat;"><a style="display:block; height:660px;" href="'.$var['link'].'"></a></li>';
      }else{
