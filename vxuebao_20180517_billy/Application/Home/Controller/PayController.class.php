...
class PayController extends Controller {
    ...
    public function index() {
        if (C("pay_type") == 2) {
			...
			$userid_int=(int)session('userid');
            $user_info = M('users')->where(array('userid_int'=>$userid_int))->find();
...
' . $html_text);
    ...
    public function usersurl() {
        ...
        \Think\Log::write($verify_result,'verify_result');
        $msg = '';
        if ($verify_result) {
...
' . var_export($_POST, true));
    ...
    function deal_trade_ok($out_trade_no) {
        ...
        if ($curinfo['pay_status'] == 0) {
            ...
            M('users')->where(array('userid_int' => intval($curinfo["user_id"])))->setInc('xd', $xd_value);
...
');
            ...
            $adddata = array('userid_int' => intval($curinfo["user_id"]), 'sceneid' => 0, 'remark' => $other_arr['name'] . '购买秀点（商品【' . $goodsInfo["goods_name"] . '】）', 'opttime' => time(), 'xd' => $xd_value, 'biztype' => 1, 'biztitle' => '购买秀点');
            M('xdlog')->add($adddata);
            \Think\Log::write(D('')->getLastSql() . '
');
        }
