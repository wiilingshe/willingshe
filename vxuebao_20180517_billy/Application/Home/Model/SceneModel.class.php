...
class SceneModel extends Model {
    ...
    public function addscene() {
        ...
        //checkAllow_nums();
        //判断是否需要审核
        $user_data = M('Users')->where(array('userid_int'=>$userid))->getField('company_ext');
        $company_ext = json_decode($user_data,'true');
        if($company_ext['is_audit'] == 'true'){
            $datainfo['is_show'] = 0;
        }
        $m_scene = M('Scene');
        ...
        $datainfo['scenename_varchar'] = str_replace(array("\"","'"), array("\\\"","\'"), $datas['name']);
		...
		$datainfo['islongpage'] = intval($datas['islongpage']);
		$datainfo['font'] = $_scene_sysinfo[0]['font'];
    ...
    public function addscenebysys() {
		...
		$datainfo['music_url'] = $_scene_sysinfo[0]['music_url'];
        ...
        $datainfo['shenhe'] = 0;
		$datainfo['islongpage'] = (int)$_scene_sysinfo[0]['islongpage'];
		$datainfo['font'] = $_scene_sysinfo[0]['font'];
        ...
        if ($result1) {
            ...
            foreach ($_scene_syspageinfo as $vo) {
                ...
                $datainfo2['properties_text'] = $vo['properties_text'];
    ...
    public function transfer() {
        ...
        if ($usr) {
			...
			$datainfo['shenhe'] = $_scene_sysinfo[0]['shenhe'];
			$datainfo['islongpage'] = (int)$_scene_sysinfo[0]['islongpage'];
			$datainfo['font'] = $_scene_sysinfo[0]['font'];
			
			$datainfo['is_public'] = $_scene_sysinfo[0]['is_public'];
    ...
    public function addscenebycopy() {
        ...
        $datainfo['movietype_int'] = $_scene_sysinfo[0]['movietype_int'];
        $datainfo['scenetype_int'] = (int)$_scene_sysinfo[0]['scenetype_int'];
        $datainfo['ip_varchar'] = get_client_ip();
        $datainfo['thumbnail_varchar'] = $_scene_sysinfo[0]['thumbnail_varchar'];
        $datainfo['musicurl_varchar'] = $_scene_sysinfo[0]['musicurl_varchar'];
		$datainfo['music_url'] = $_scene_sysinfo[0]['music_url'];
    ...
    public function savepage() {
        ...
        $datainfo['longpage']   =$datas['longpage']?$datas['longpage']:null;
        $where['userid_int'] = $userid;

        $wheredata['userid_int'] = $userid;
       ...
       $datainfo['content_text'] = json_encode($datas['elements']);
        ...
        $sysdatainfo['properties_text'] = $datainfo['properties_text']; 
        $redd = $m_scenepagesys->data($sysdatainfo)->where(array('sceneid_bigint' => $datas['sceneId']))->save();
        ...
        if (C('JS_VISION') == '3.4') {
            ...
            $bgdatainfo['musicurl_varchar'] = $datas['scene']['bgAudio']['url'];
			 $bgdatainfo['musicurl_varchar'] = $datas['scene']['bgAudio']['music_url'];
        ...
        $bgdatainfo['is_tpl'] = 0;
		$bgdatainfo['font'] = json_encode($datas['font']);
		if(!$bgdatainfo['font']){
			$bgdatainfo['font']='[]';
		}
        $bgdatainfo['updateTime'] = time();
        //判断子账号修改是否需要审核
        $udatares = M('Users')->field('integral_num,company_ext')->where(array('userid_int' => $userid))->find();
        $auditres = json_decode($udatares['company_ext'],true);
        if($auditres['is_audit'] == 'true'){
            $resshow = $m_scene->where($where)->getField('is_show');
            if($auditres['is_integral'] == 'true' && $resshow == 1){
                $integral_num['integral_num'] = $udatares['integral_num'] - $auditres['integral_num'];
                M('Users')->where(array('userid_int'=>$userid))->save($integral_num); 
            }
            $bgdatainfo['is_show'] = 0;
            if($resshow == 1){
                $bgdatainfo['showstatus_int'] = 0;
            } 
        }

        $m_scene->data($bgdatainfo)->where($where)->save();
		//echo $m_scene->getlastsql();die;
        echo json_encode(array("success" => true,
            "code" => 200,
            "msg" => "success",
            "obj" => $result1,
    ...
    public function savesetting() {
        ...
        $datainfo['showstatus_int'] = intval($datas['status']);
        ...
        $datainfo['applyGuanchan'] = $datas['isShow'];
        $music = json_decode($datas['bgAudio'], true);

        $datainfo['musicurl_varchar'] = $music['url'];
		//$datainfo['music_url'] = json_encode($datas['obj']['music_url']);


        $datainfo['tagid_int'] = $datas['tagId'] ? intval($datas['tagId']) : 0;

        $son = M('users')->where(array('userid_int' => $userid))->getField('type');
        if($son != 2){
            $where['userid_int'] = $userid;
        }

        $m_scene->where($where)->save($datainfo);
