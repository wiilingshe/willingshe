...
function bgSoundOpen() {
	...
	if(bgSoundOpenState==0){
		...
		acnfc("bgSound", "img", 1, "d", "none");
		//acnfc("bgSound", "img", 1, "s", "img/music_on.png");
	}else{
		//afc("myAudio","s","");
		myAudio.pause();
		bgSoundOpenState = 0;
		acnfc("bgSound", "img", 0, "d", "none");
		acnfc("bgSound", "img", 1, "d", "block");
...
function PN()
{
	...
	{
		case 1:
			if(s("PN").style.top=="-"+Number(pageTotal-2)+"00%"||s("PN").style.top=="-"+Number(pageTotal-1)+"00%"){
			     afc("XB","d","none");
			}else{
				afc("XB","d","block");
			}
