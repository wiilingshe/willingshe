<template>
	<div class="serAsk">
		<div class="type">
			<ul>
				<li :class="{choose:choosed==1}" @click="selecetType(1)">课程问答</li>
				<li :class="{choose:choosed==2}" @click="selecetType(2)">视频问答</li>
		...
		<div v-show="choosed==1">
			...
			<ul style="height:calc(100vh - 1.5rem);overflow:scroll;-webkit-overflow-scrolling:touch" v-infinite-scroll="loadMore" infinite-scroll-disabled="false" infinite-scroll-distance="10" infinite-scroll-immediate-check="true">
				<li v-for="item in arr" :key="item.id">
		  			<teacherList :info="item,choosed,ifdaoshi"></teacherList>
				</li>
...
<script>
	...
	import teacherList from '@/components/ask/teacherList'
  ...
  export default{
		...
		created(){
			...
			this.noShare();
			this.postData('post','shop-v2-ask_teacher.html',{source_type:this.type,page:this.pageNo,status:this.status}).then(res => {
				this.arr = res.data;
		...
		methods:{
			...
			chooseList(type,status){
				...
				this.postData('post','shop-v2-ask_teacher.html',{source_type:type,page:this.pageNo,status:status}).then(res => {
					this.arr = res.data;
					this.listLength = res.data.length;
					if(res.data.length == 0){
						this.load = false;
			...
			chooseList2(type,status){
				...
				this.postData('post','shop-v2-ask_teacher.html',{source_type:type,page:this.pageNo2,status:status}).then(res => {
					this.arr2 = res.data;
			...
			chooseList3(type,status){
				...
				this.postData('post','shop-v2-ask_teacher.html',{source_type:type,page:this.pageNo3,status:status}).then(res => {
					this.arr3 = res.data;
			...
			loadMore2(){
				if(this.listLength2 != 0){
					...
					this.postData('post','shop-v2-ask_teacher.html',{source_type:this.type2,page:this.pageNo2,status:this.status2}).then(res => {
			...
			loadMore3(){
				if(this.listLength3 != 0){
					...
					this.postData('post','shop-v2-ask_teacher.html',{source_type:this.type3,page:this.pageNo3,status:this.status3}).then(res => {
