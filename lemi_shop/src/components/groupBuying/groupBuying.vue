    ...
    <div class="groupBuying_main" v-if="!isIn && isRecord">
      ...
      <div class="groupBuying_main-item" v-for="(i,index) in buyArr">
        ...
        <div class="groupBuying_main-right">
          <span class="groupBuying_main-btn" v-if="!i.is_full && !i.isClick" @click="alertBox(i.grp_id,index)">
            来抱团
          </span>
...
//                        已结束状态
    ...
    methods:{
          ...
          MessageBox({
            ...
            if(action == 'confirm'){
              if(window.location.href.indexOf('vid') != -1){
                this.$router.push({path:'/toPay',query:{'p_id':this.alertObj.pid,'type':'7',shipin:this.$route.query.vid,group_type:2}});
              }else{
                this.$router.push({path:'/toPay',query:{'p_id':this.alertObj.pid,'type':'7',course:this.$route.query.ser_id,group_type:2}});
              }
            }
          });
      ...
      toPay(Pid){
        if(window.location.href.indexOf('vid') != -1){
          this.$router.push({path:'/toPay',query:{'p_id':Pid,'type':'7','shipin':this.$route.query.vid,group_type:1}});
        }else{
          this.$router.push({path:'/toPay',query:{'p_id':Pid,'type':'7','course':this.$route.query.ser_id,group_type:1}});
        }
      }
    },
    beforeDestroy(){
