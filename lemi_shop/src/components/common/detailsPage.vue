  ...
  <div id="box" class="detailsPage">
    ...
    <div class="liwu_box" v-show="liwu">
      ...
      <div class="liwu_number">
        <div>赠送数量</div>
        <div class="liwu_add">
          <span class="jian" @click="toReduce()">-</span><!----><span class="num">{{liwuNum}}</span><!----><span
          class="jia" @click="toAdd()">+</span>&nbsp份
        </div>
    ...
    <div class="kefuCode" v-show="invite">
      ...
      <!-- <div class="invite">
				<div class="invite_pic">
					<img class="img222" :src="fenmian">
				</div>
				<div class="invite_one">
					<div class="invite_tou">
						<img :src="inviteData.face">
					</div>
					<div class="invite_table">
						<div>{{inviteData.nickname}}</div>
						<div>给你一张学习明信片</div>
					</div>
					<img class="img111" src="../../../static/img/invite/inviteyin.png">
				</div>
				<div class="invite_title">{{inviteData.title}}</div>
				<div class="invite_name">主讲人：{{inviteData.owner_name}}</div>
				<div style="width:100%;text-align:center;margin-top:0.4rem;">
					<img :src="inviteData.qr_code" style="width:1.365rem;height:1.365rem;text-align:center">
					<div style="margin-top:0.05rem;">长按识别二维码参加课程</div>
				</div>
			</div> -->
    </div>
    <div v-show="!ifGet">
      <div class="sharetop" v-if="audiopic" style="width:100%;height:3.41rem;overflow:hidden;position:relative;">
      ...
      <div class="headers" v-show="headers" :class="{height1:height1}">
        ...
        <div class="picDan" v-if="picDan">
          <img :src="fenmian" v-show="fenmian != null" onerror="this.src=&quot;/static/img/pic_addfengmian.png&quot;">
          <div class="bg06"></div>
          ...
          <i class="iconfont icon-play" @click="paly()" v-show="playIcon && !ifstudydao && ifplayicon"></i>
          <!--<i class="iconfont icon-qiapianx qiapianx" style="color:#fff" @click="shareget()"></i>-->

          <!--<div class="share" v-show="share1" @click="inviteclick()">-->
          <div class="share" @click="inviteclick()">
						<span>
							<i class="iconfont icon-qiapianx" style="color:#fff"></i>
							{{dist_money}}
            ...
            <div class="curBox">
              ...
              <ul class="listBox_inner">
                <li v-for="(item,index) in innerBox" class="listBox_li"
                  ...
                  <span
                    :class="{noBuy:item.is_foreshow==1?true:ifFree==1?false:isBuy==2?false:item.is_free==1?false:true}">{{item.title}}</span>
                  <i class="iconfont icon-play"
      ...
      <textarea class="note" ref="biji2" v-show="note" placeholder="请输入笔记">
        ...
        <div id="navBox" v-if="isOn && ifBuy" @click="toPayDemo()">
          <div style="width:100%;display:table;height:0.8rem;" v-if="ifSale == true">
            <div style="display:table-cell;vertical-align: middle;">
              <span style="font-size:0.24rem;">立即购买</span><span
              style="font-size:0.24rem;">( &yen {{lists.sales_fee}})</span>
              <div style="text-decoration:line-through;font-size:0.22rem;color:#ffa5b5">&yen{{lists.total_fee}}</div>
            </div>
          </div>
          <div v-if="ifSale == false" style="width:100%;display:table;height:0.8rem;">
            <div style="display:table-cell;vertical-align: middle;">
              <div style="font-size:0.24rem;">立即购买<span>( &yen {{lists.total_fee}})</span></div>
      ...
      <div class="ask ask1" v-show="note" @click="savebi()" style="background:#1a91d5">
        保存笔记
      </div>
  ...
  export default {
    ...
    created(){
      ...
      if (href.indexOf('vid') != -1) {
        ...
        // this.postData('post','/shop-v1-ask_get_video_detail_lst.html',{vid:idd,page:this.pageNo,status:'2'}).then(res => {
        // 	this.load5 = false;
        // 	this.arr = res.data;
        ...
        // this.postData('post','/shop-v1-ask_get_series_detail_lst.html',{ser_id:idd,page:this.pageNo,status:'2'}).then(res => {
        // 	this.load5 = false;
        // 	this.arr = res.data;
        // 	if(res.data.length == 0){
        // 		this.load6 = true;
...
//        关联商品结束
    ...
    methods: {
      ...
      // 保存笔记
      savebi(){
        var val = this.$refs.biji2.value;
        if (window.location.href.indexOf('ser_id') != -1) {
          this.postData('post', '/shop-v1-user_notes.html', {
            type: '2',
      ...
      biji(){
        if (this.note == false) {
          this.note = true;
          if (window.location.href.indexOf('ser_id') != -1) {
            this.postData('post', '/shop-v1-user_get_notes.html', {
              type: '2',
              id: this.$route.query.ser_id
            }).then(res => {
              if (res.data.status == 1) {
                this.$refs.biji2.value = res.data.notes
              } else {
                Toast({
                  message: res.data.msg,
                  duraction: 1000
            ...
            this.postData('post', '/shop-v1-user_get_notes.html', {type: '1', id: this.$route.query.vid}).then(res => {
              if (res.data.status == 1) {
                this.$refs.biji2.value = res.data.notes
              } else {
                Toast({
      ...
      activeTil($event, index, ifFree, number, foreshow){
        ...
        this.picDan = true;
        this.videoUrl = '';
        $('iframe').css('height', '3.41rem');
        $('.headers').css('height', '3.41rem');
        ...
        for (let i = 0; i < $('.listBox_li').length; i++) {
          if ($('.listBox_li').eq(i).find('i').css('color') != 'rgb(153, 153, 153)') {
            $('.listBox_li').eq(i).find('i').css('color', '#666');
          }
        }
        $($event.currentTarget).find('i').css('color', '#5e96d0');
...
//      },
    ...
    components: {
      detailquestions, nodata, loadNo, groupBuying, goodsRelation
    },
    destroyed(){
    },
    watch: {
...
<style rel="stylesheet/scss" lang="less">
  ...
  .detailsPage .mint-navbar .mint-tab-item.is-selected {
    border-bottom: 1px solid #5e96f0;
    color: #5e96f0;
    margin-bottom: -1px;
  }
