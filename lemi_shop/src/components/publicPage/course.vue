		...
		<div class="main">
				...
				<mt-tab-container v-model="selected">
					  <mt-tab-container-item id="1">
					  	<div class="liebiao" v-infinite-scroll="loadMore" infinite-scroll-disabled="false" infinite-scroll-immediate-check="false" infinite-scroll-distance="0">
						  	<div class="main-box" v-for="i in arr">
						  		<router-link :to="{path:'/details',query:{ser_id:i.ser_id}}">
									...
									<div class="right">
										<div>{{ i.ser_title}}</div>
										<div style="margin-top: 0.25rem;">导师:{{ i.username}}</div>
					  ...
					  <mt-tab-container-item id="2">
					  	<div class="liebiao" v-infinite-scroll="loadMore1" infinite-scroll-disabled="false" infinite-scroll-immediate-check="false" infinite-scroll-distance="0">
						  	<div v-if="videoData">
								<div class="middle">
									<img src="../../../static/img/goods_fill.png">
									<div class="zi">书架空空如也</div>
						    ...
						    <div class="main-box" v-for="j in arr2">
							   	<router-link :to="{path:'/details',query:{vid:j.vid}}">
									...
									<div class="right" style="position: relative;">
										<div>{{ j.title }}</div>
										<div style="position: absolute;bottom: 0.2rem;">导师:{{ j.username }}</div>
...
<script>
	...
	export default {
		...
		created(){
			...
			this.noShare();
			if(this.selected == 1){
				this.getData('get','/shop-v1-lesson_series_lesson.html?page=1').then(res => {
				...
				this.getData('get','/shop-v1-lesson_video_lesson.html?page=1').then(res => {
					if(res.data.length == 0){
						this.videoData = true;
					}else{
		...
		methods:{
			...
			loadMore(){
				this.page1++;
				this.getData('get','/shop-v1-lesson_series_lesson.html?page='+this.page1).then(res => {
					this.arr.concat(res.data)
				})
			},
			loadMore1(){
				this.page2++;
				this.getData('get','/shop-v1-lesson_video_lesson.html?page='+this.page2).then(res => {
					this.arr2.concat(res.data)
				})
			}
		},
		components:{footer1},
