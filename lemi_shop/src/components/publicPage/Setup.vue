  ...
  <div class="box setUp">
    <div class="boxNext" v-show="!nextShow">
      <div class="portrait">
        ...
        <div><i class="iconfont icon-dayuhao1" style="position: relative;top: 0.55rem;"></i></div>
      </div>
      <div class="name">
        <div>昵称</div>
        <div>{{ arr.nickname }}<i class="iconfont icon-dayuhao1"></i></div>
      </div>
    ...
    <span style="color:#999;font-size:0.26rem;text-align: center;width:100%;position: absolute;bottom: 0.2rem;">
			<footer1></footer1>
		</span>
  </div>
</template>
  ...
  export default{
    ...
    created(){
      ...
      this.getData('get', '/shop-v1-user_get_ext_info.html').then(res => {
        if (res.status == 200) {
          ...
          this.radioValue = toString(res.data.sex);
          if (res.data.sex == 1) {
            this.sexNmae = '男'
    ...
    methods: {
      ...
      submit(){
        ...
        this.postData('post', '/shop-v1-user_save_ext_info.html', {
          'birthday': this.arr.birthday,
          'sex': this.sex1,
          'location': this.introduce
        }).then(res => {
          if (res.status == 200) {
