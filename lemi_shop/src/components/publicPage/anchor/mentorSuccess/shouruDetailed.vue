<template>
	<div class="shouruDetailed_box">
		<div class="shouruDetailed_header">
			<div>单号</div>
			<div>类型</div>
			<div>交易金额</div>
			<div>时间</div>
		</div>
		<div class="shouruDetailed_main" v-infinite-scroll="loadMore"
...
<script>
	...
	export default{
		...
		created(){
			...
			this.getData('get','/shop-v1-shop_get_shop_wallet.html?page=1&pageSize=12').then(res => {
				if(res.status == 200){
					this.arr = res.data;
				}
			})
		...
		methods:{
			loadMore(){
				this.page++;
				var url = '/shop-v1-shop_get_shop_wallet.html?pageSize=12&page='+this.page;
