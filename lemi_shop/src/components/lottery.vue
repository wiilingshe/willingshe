<template>
	<div class="lottery">
		<div class="title-com" style="position: relative;"><img style="width: 6rem;margin-left: calc(50% - 3rem);margin-top: 0.5rem;" src="data:image/png;base64," alt="" class="titlebg"> <div class="text" style="position: absolute;top: 0.65rem;text-align: center;width: 100%;">{{arr3.activity_title}}</div></div>
		...
		<div class="share">
			<p @click="shareDemo()">{{arr3.activity_intro}}</p>
		</div>
      ...
      <div class="price" v-for="priceitem in price" style="display: inline-block;width:50%;padding-bottom:0.14rem; ;text-align: center">
		...
		<ul style="width: 84%;position: fixed;top: 3rem;left:8%;z-index: 222;" v-show="tip">
		 ...
		 <li style="height:auto;background-color:#787878;text-align:center;padding-top:0.3rem;color:#DCC722;padding-bottom:0.3rem;">
		 	...
		 	<div style="padding: 0rem 0.1rem;">{{code==''?'':'恭喜您中奖，兑换码在奖品中心可以查看'}}</div>
		 </li>
...
export default{
	...
	created(){
		...
		this.getData('get','/shop-v1-index_basemsg.html').then(res => {
			var pic1 = res.data.logo;
			console.log(pic1)
			self.postData('post','/shop-v1-draw_index.html',{activity_id:self.$route.query.activity_id}).then(res => {
				self.arr3 = res.data;
				self.allTitle = res.data.activity_title;
				var desc1 = res.data.activity_intro
				// 获取抽奖信息
				self.postData('post','/shop-v1-pay_jssign.html',{url:window.location.href}).then(res => {
