  ...
  <div class="contentPay" style="background-color:#f1f2f6;width:100%;overflow-x:hidden">
        ...
        <li>
          <div class="wenzi">
            ...
            <div class="yuanjia" style="width: 100%">
              &yen;{{price}}
              <span v-if="shopNum"
                    style="width: 75%;text-align: right;display: inline-block;font-size: 0.24rem;color: #999;">x&nbsp{{shopNum}}</span>
            </div>
          </div>
        </li>
      </ul>
    ...
    <div class="yihang" style="background-color:#fff" v-if="yuequ">
      <ul>
        <li>余额抵扣</li>
        <li>
          <input type="text" name="" v-model="realFee"
                 style="width: 1.2rem;height: 0.4rem;text-align:center;border:1px solid #999;outline-style: none;">
        </li>
      </ul>
    </div>
    <div class="yihang" style="background-color:#fff">
      <ul>
        ...
        <li style="font-weight: 500;">&yen{{finFee}}<span v-if="points != ''?true:false" style="color:#666"> ({{points}}积分)</span>
        </li>
      </ul>
    </div>
    ...
    <div class="yihang" style="background-color:#fff" v-if="package">
      <ul>
        ...
        <li style="font-weight: 500;">-{{newPic}}</li>
      </ul>
    </div>
    <div v-if="this.$route.query.askId">
      ...
      <div class="payWay1" style="background-color:#fff;" v-for="item in paytype">
        <ul>
          <li v-if="item.pay_type==1" style="border-top:0;"><img src="../../static/img/wechat.png"><span>微信支付</span>
          </li>
          <li v-if="item.pay_type==0"><img src="../../static/img/jifen.png"><span>积分支付<span
            style="color:#999;margin-left:0;font-size:0.23rem;">（{{points1}}积分）</span></span></li>
          <li>
						<span class="radio">
...
<script>
  ...
  export default{
    ...
    created(){
      ...
      this.noShare();
      if (window.location.href.indexOf('askId') != -1) {
        this.postData('post', '/shop-v2-ask_get_fee.html', {
          fee_type: this.$route.query.order_type,
          is_public: this.$route.query.is_public
        }).then(res => {
        ...
        this.postData('post', '/shop-v1-pay_pay_msg.html', {
    ...
    methods: {
      ...
      toPay(){
          ...
          if (type == 'ser_id') {
            self.$router.push({
              ...
              query: {
                ...
                'order_type': self.$route.query.order_type,
                orderNo: order_no,
                member_id: member_id
              }
            })
        ...
        if (window.location.href.indexOf('vid') != -1 && window.location.href.indexOf('askId') == -1) {
          this.postData('post', '/shop-v1-pay_order.html', {
            vid: this.$route.query.vid,
            type: '1',
            balance: this.realFee,
          ...
          this.postData('post', '/shop-v2-ask_order.html', {
            id: this.$route.query.askId,
            type: this.$route.query.type,
            balance: this.realFee,
            other_type: this.$route.query.order_type,
            pay_type: this.toggle
          }).then(res => {
            if (res.data.status == 1) {
              Indicator.close();
              if (this.toggle == 0) {
                ...
                if (res.data.is_need_pay == 1) {
                  ...
                  if (window.location.href.indexOf('vid') != -1) {
                    this.$router.push({
                      path: '/successPay',
                      query: {'vid': this.$route.query.vid, askId: this.$route.query.askId}
                    })
                  } else {
                    this.$router.push({
                      path: '/successPay',
                      query: {'ser_id': this.$route.query.ser_id, askId: this.$route.query.askId}
                    })
                  }
          ...
          this.postData('post', '/shop-v1-pay_order.html', {
            ...
            if (res.data.status == 1) {
              ...
              if (res.data.is_need_pay == 0) {
                ...
                this.$router.push({
                  path: '/payWay',
                  query: {type: this.$route.query.type, 'cs_id': this.$route.query.cs_id, orderNo: res.data.order_no}
                });
              }
            } else if (res.data.status == 0) {
              Indicator.close();
              ...
              if (res.data.is_need_pay != 0) {
                    ...
                    function (res) {
                      if (res.err_msg == "get_brand_wcpay_request:ok") {
                        ...
                        if (window.location.href.indexOf('shipin') != -1) {
                          self.$router.push({
                            path: '/successPay',
                            query: {p_id: self.groupId, 'shipin': self.$route.query.shipin}
                          });
                        } else {
                          self.$router.push({
                            path: '/successPay',
                            query: {p_id: self.groupId, 'course': self.$route.query.course}
                          });
                        }
                      } else {
                        if (window.location.href.indexOf('shipin') != -1) {
                ...
                this.$nextTick(function () {
                  ...
                  if (window.location.href.indexOf('shipin') != -1) {
                    this.$router.push({
                      path: '/successPay',
                      query: {p_id: this.groupId, 'shipin': this.$route.query.shipin}
                    });
                  } else {
                    this.$router.push({
                      path: '/successPay',
                      query: {p_id: this.groupId, 'course': this.$route.query.course}
                    });
                  }
                });
              }
    ...
    computed: {
      newPic: function () {
        return this.accSub(this.price, this.zongjia);
      }
    }
  }
</script>
