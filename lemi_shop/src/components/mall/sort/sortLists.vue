	...
	<div class="sortLists">
		...
		<div class="lists">
			<ul>
				<li>
					<div class="idLists">
						<div @click="getlits(item.cat_id)" class="idlist" v-for="(item,index) in arr" :data-id="item.cat_id" :style="{'background-color':catId ==item.cat_id?'#fff':'#fff',color:catId ==item.cat_id?'#ef4949':'#333'}">
							{{item.cat_name}}
						</div>
					</div>
				</li>
...
//      跳转分类商品页
			tolists(id){
				this.$router.push({path:'goodsLists',query:{catId:id,type_id:this.$route.query.type_id}});
			},
//      搜索跳转分类商品页
			search(){
				this.$router.push({path:'goodsLists',query:{name:this.$refs.name.value,type_id:this.$route.query.type_id}});
			},
//      是否显示原导航栏
      listenToMyBoy(is){
        if(is != undefined){
