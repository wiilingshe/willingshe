<template>
	<div class="lookrecord" v-infinite-scroll="loadMore"
  ...
  infinite-scroll-distance="0">
  		...
  		<div style="background-color:#fff;overflow:auto;">
			<div class="lookrecord-list" v-for="(i,index) in list" @click="datails(i.art_id,i.article_type)" :style="{'border-bottom':list.length==index+1?'none':'1px solid #eee'}">
...
<script>
	...
	export default{
		...
		created(){
			document.title = '我的收藏';
			this.getData('post','/shop-v1-article_get_collect_lst.html',{page:1,type:1}).then(res => {
		...
		methods:{
			loadMore(){
				if(this.ifload == true){
					...
					this.getData('post','/shop-v1-article_get_collect_lst.html',{page:this.page,type:1}).then(res => {
