<template>
	<div class="goodDetails" :class="selectSkuis?'goodHidden':''">
		<div class="lunbo">
          ...
          <tr @click="toshopcart()">
			...
			<mt-swipe :auto="0" style="z-index: 2">
				<mt-swipe-item v-if="list.video != null">
			  	<video ref="headVideo" style="width: 100%;height: 6.4rem;" :poster="list.pic" :src="list.video" controls="controls"></video>
          <!--<img style="position:absolute;left:0;top: 0;width: 100%;height: 6.4rem;" :src="list.pic" v-if="pauseIs"  @click="videoPlay()">-->
			  ...
			  <mt-swipe-item v-for="(item,index) in arr" :key="index">
			  	<img style="width:100%;height:6.4rem;" :src="item" onerror="this.src=&quot;/static/img/onerr2.png&quot;">
			  </mt-swipe-item>
			</mt-swipe>
		</div>
		<div class="info">
				...
				<ul class="ul4">
					<li>
						<div style="position:relative;top:-0.5rem;padding:0.1rem;background-color:#fff;margin-left: 0.17rem;margin-right: 0.17rem;">
							<img class="img2" :src="arr" onerror="this.src=&quot;/static/img/onerr2.png&quot;">
						</div>
					</li>
					<li>
						<div class="xintable">
      ...
      <div class="addressWrap">
        ...
        <div class="addressList1" :class="{'selectAddress':item.is_default==1?true:false}" @click="selectAddress(item.address_id,$event)" v-for="item in addressLists">
          <ul class="ul6">
            <li>
          ...
          <mt-swipe-item  v-for="(item,index) in bigImg" :key="index">
            <div style="display: table;width:100%;height: 100%;">
              <div style="display: table-cell;width:100%;vertical-align: middle;height: 100%;">
                <img  class="customAdv_bigimg" :src="item" onerror="this.src=&quot;/static/img/onerr2.png&quot;" >
              </div>
            </div>
          </mt-swipe-item>
        </mt-swipe>
...
<script>
	...
	export default{
		...
		created(){
			...
			// this.shepei();
			var self = this;
			this.postData('post','/shop-g1-goods_show.html',{goods_id:this.$route.query.goods_id}).then(res => {
        if(res.status == 200){
//          console.log(res.data.length);
          if(res.data.length == undefined){
            ...
            this.list = res.data;
                this.thisCode = res.data.code;
//            console.log(this.thisCode);
                this.arr = [res.data.pic];
                this.arr2 = res.data.sku.spec_list ? res.data.sku.spec_list:[];
...
//				let firstName = '选择 ';
                ...
                if(res.data.promotion_type == 1){
                  var now = new Date().getTime();
                  now = (now/1000).toFixed(0)
                  if(parseInt(res.data.sale_start_time)<parseInt(now) && parseInt(now)<parseInt(res.data.sale_end_time)){
                    this.ifSalse = true;
                    this.thisPrice = res.data.sale_price*res.data.shop_price;
                    this.thisPrice = this.thisPrice.toFixed(2);
                  }else{
                    this.thisPrice = res.data.shop_price;
                  }
                }else{
                  this.thisPrice = res.data.shop_price;
                }
...
//      点击更多事件
      ...
      //    跳转首页
        ...
        this.postData('post','/shop-v1-index_get_root.html').then(res => {
          ...
          this.$router.push({path:'/mallHome',query:{type_id:4,root_id:id}});
        })
			},
      //    跳转订单中心
			toMine(){
        this.postData('post','/shop-v1-index_get_root.html').then(res => {
          ...
          this.$router.push({path:'/orderCenter',query:{type_id:4,root_id:id}});
        })
			},
      //    跳转购物车
      toshopcart(){
        this.postData('post','/shop-v1-index_get_root.html').then(res => {
          ...
          this.$router.push({path:'/shopCart',query:{type_id:4,root_id:id}});
        })
