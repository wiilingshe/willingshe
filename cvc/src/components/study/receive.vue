<template>
	<div class="receive_box" :class="returnData?'':'receive_box_no'">
		<div class="receive_main" v-if="0">
    ...
    <div class="receive_wrap" v-if="1">
      <div class="give_one" v-if="isReceive">
        <p style="font-weight: bold;">礼物已成功领取</p>
        <p style=" font-size: 0.24rem;color: #999999;margin-top: 0.2rem">请进入【我的】- 【赠礼】中查看~</p>
      </div>
      <div class="give_one" v-if="!isReceive && data.is_receive==1">
        <p style="font-weight: bold;">礼物已成功领取</p>
        <p style=" font-size: 0.24rem;color: #999999;margin-top: 0.2rem">请进入【我的】- 【赠礼】中查看~</p>
      </div>
      ...
      <div class="main">
        ...
        <div class="main_title">
          <div style="height: 1.5rem;width: 2.5rem;background: #eee;overflow: hidden;margin-left: 0.2rem;border-radius: 0.06rem;">
      ...
      <div class="btn" v-if="!isReceive && data.is_receive==0" @click="lingqu()">
        <span >领取赠礼</span>
      </div>
      <div class="btn" v-if="isReceive" @click="goPage()">
        <span>开始学习</span>
      </div>
      <div class="btn" v-if="!isReceive && data.is_receive==1" @click="goPage()">
        <span>开始学习</span>
      </div>
      ...
      <div class="give_details" v-if="!isReceive && 0">
		...
		<div v-if="0" style="color: rgb(153, 153, 153); font-size: 0.26rem; text-align: center;padding-top: 0.2rem;">
      <div>小象SaaS提供技术支持</div> <div>www.saasxx.vip</div>
    </div>
	</div>
</template>
	 ...
	 export default {
        ...
        Indicator.open({
          spinnerType: 'snake'
    		...
    		this.postData('post','/shop-v1-order_get_receive_lesson_detail.html',{'order_no':this.$route.query.order_no,'member_id':this.$route.query.member_id,'type':this.$route.query.type,'ser_id':this.$route.query.ser_id}).then(res => {
          Indicator.close();
...
//          this.isReceive = true;
    			if(this.$route.query.type=='video'){
    				this.postData('post','/shop-v1-order_add_receive_lesson.html',{'order_no':this.$route.query.order_no,'member_id':this.$route.query.member_id,'type':this.$route.query.type,'id':this.$route.query.vid}).then(res => {
		    			if(res.status == 200){
                if(res.data.status == 1){
                  ...
                  Toast({
    				...
    				this.postData('post','/shop-v1-order_add_receive_lesson.html',{'order_no':this.$route.query.order_no,'member_id':this.$route.query.member_id,'type':this.$route.query.type,'id':this.$route.query.ser_id}).then(res => {
	    				if(res.status == 200){
                if(res.data.status == 1){
                  this.isReceive = true;
                  Toast({
