<template>
	<div id="box" style="height:100%;" class="classfenlei">
		<div class="headerss">
			<div class="inner">
					...
					<div id="kecehng">
						<div v-show="kecheng" style="padding:0 0.2rem;">
							<div class="inner_kecheng clearfix" v-for="x in navArrTwo">
								<router-link :to="{path:'/classify-shipinsearch',query:{navsnum:1,catId:x.cat_id}}">
									<p>{{x.cat_name}}</p>
								</router-link>
								<div class="danpin" style="background:#fff;">
									<div v-for="item in x.child" class="zizi">
										<router-link :to="{path:'/classify-shipinsearch',query:{navsnum:1,catId:item.cat_id}}">
						...
						<div v-show="danpin" style="padding:0 0.2rem;">
							<div class="inner_kecheng clearfix" v-for="i in danpinArr">
								<router-link :to="{path:'/classify-shipinsearch?navsnum=2',query:{navsnum:2,catId:i.cat_id}}">
									<p>{{i.cat_name}}</p>
								</router-link>
								<div class="danpin" style="background:#fff;">
									<div v-for="j in i.child" class="zizi">
										<router-link :to="{path:'/classify-shipinsearch?navsnum=2',query:{navsnum:2,catId:j.cat_id}}">
...
<script>
	...
	export default {
      ...
      allFn:function(){
        if(this.navsNum==1){
          this.$router.push({path:"/classify-shipinsearch",query:{navsnum:1}})
        }
        if(this.navsNum==2){
          this.$router.push({path:"/classify-kechengsearch",query:{navsnum:2}})
        }
        if(this.navsNum==3){
        ...
        this.all=false;
			...
			//分类点击传参跳转
			clickFn:function(){
				this.$router.push({path:'/classify-shipinsearch',query:{catId:1}});
			},
			...
			jumpFn:function(){
				if(this.navsNum==1){
					...
					if(this.vals!=''){
						this.$router.push({path:"/classify-shipinsearch",query:{navsnum:1,title:this.vals}})
					}else{
						this.$router.push({path:"/classify-shipinsearch",query:{navsnum:1}});
					}
				...
				if(this.navsNum==2){
					this.vals = this.vals.trim();
					if(this.vals!=''){
						this.$router.push({path:"/classify-kechengsearch",query:{navsnum:2,title:this.vals}})
					}else{
						this.$router.push({path:"/classify-kechengsearch",query:{navsnum:2}});
					}
